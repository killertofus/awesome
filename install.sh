#!/usr/bin/env bash
sudo sed -i -e 's/debug/!debug/g' /etc/makepkg.conf -e '/#MAKEFLAGS="5"/c\MAKEFLAGS="--jobs=$(nproc)"' /etc/makepkg.conf -e '32 a ILoveCandy' /etc/pacman.conf -e '/Color/s/^#//g' /etc/pacman.conf -e '/#NPROC="2"/c\ NPROC="$(nproc)"'/etc/makepkg.conf
sudo -v
