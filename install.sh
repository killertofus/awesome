#!/usr/bin/env bash
sudo sed -i -e '/Color/s/^#//g' /etc/pacman.conf -e '31 a ILoveCandy' /etc/pacman.conf
sudo sed -i -e  '/#MAKEFLAGS="2"/c\MAKEFLAGS="--jobs=$(nproc)"' /etc/makepkg.conf -e '93,95s/debug/!debug/g' /etc/makepkg.conf -e '/#NPROC="2"/c\ NPROC="$(nproc)"'/etc/makepkg.conf
sudo -v
