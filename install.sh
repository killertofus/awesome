#!/usr/bin/env bash
sudo sed -i -e  '/#MAKEFLAGS="5"/c\MAKEFLAGS="--jobs=$(nproc)"' /etc/makepkg.conf -e '93,95s/debug/!debug/g' /etc/makepkg.conf -e '/#NPROC="2"/c\ NPROC="$(nproc)"'/etc/makepkg.conf
sudo sed -i -e /etc/pacman.conf -e '/Color/s/^#//g' /etc/pacman.conf -e '32 a ILoveCandy'
sudo -v
